...
class RoundaboutEnv(AbstractEnv):
    ...
    def _make_vehicles(self):
        ...
        # Incoming vehicle
        for i in list(range(0, 2)) + list(range(-1, 0)):
            destinations = ["exr", "sxr", "nxr", ]
            other_vehicles_type = utils.class_from_path(self.config["other_vehicles_type"])
            vehicle = other_vehicles_type.make_on_lane(self.road,
                                                       ("we", "sx", 1),
                                                       longitudinal=60*i + self.np_random.randn() * position_deviation,
        ...
        for i in list(range(1, 3)) + list(range(-1, 0)):
            vehicle = other_vehicles_type.make_on_lane(self.road,
                                                       ("we", "sx", 0),
                                                       longitudinal=25*i + self.np_random.randn()*position_deviation,
                                                       velocity=16 + self.np_random.randn()*velocity_deviation)
