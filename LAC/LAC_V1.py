...
class VAE(object):
    def __init__(self, ):
        ...
        z_mu, z_logvar, distribution= self.encoder(self.S)
        ...
        self.reconstruct_x, _ = self.decoder(z_mu, reuse=True)

        self.recon_loss = tf.reduce_mean(tf.pow(x_mu - self.S , 2))

        # D_KL(Q(z|X) || P(z)); calculate in closed form as both dist. are Gaussian
        self.kl_loss = 0.5 * tf.reduce_mean(tf.exp(z_logvar) + z_mu ** 2 - 1. - z_logvar,1)

        # self.kl_loss = self.wasserstein_distance(z_mu,z_logvar,mu,logvar)
        # VAE loss
        self.vae_loss = tf.reduce_mean(self.recon_loss + self.kl_loss)
        self.optimizer = tf.train.AdamOptimizer(learning_rate).minimize(self.vae_loss)
